...
//!
//! The module contains [`ScalarValue`] and [`AnyValue`] types used in
//! various operations and data types.
...
/// Wrapper for base types.
///
/// Type can be used to mix of different types within collections.
#[derive(Debug, Clone, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Deserialize), serde(untagged))]
pub enum AnyValue {
    /// `HashMap` with values of different types.
    HashMap(HashMap<String, Box<AnyValue>>),

    /// `Vec` with values of different types.
    Vec(Vec<Box<AnyValue>>),

    /// `String` value stored for specific key in [`AnyValue::HashMap`] or
    /// element in [`AnyValue::Vec`].
    String(String),

    /// `Boolean` value stored for specific key in [`AnyValue::HashMap`] or
    /// element in [`AnyValue::Vec`].
    Boolean(bool),

    /// `signed 8-bit` value stored for specific key in [`AnyValue::HashMap`] or
    /// element in [`AnyValue::Vec`].
    Signed8(i8),

    /// `unsigned 8-bit` value stored for specific key in [`AnyValue::HashMap`]
    /// or element in [`AnyValue::Vec`].
    Unsigned8(u8),

    /// `signed 16-bit` value stored for specific key in [`AnyValue::HashMap`]
    /// or element in [`AnyValue::Vec`].
    Signed16(i16),

    /// `unsigned 16-bit` value stored for specific key in [`AnyValue::HashMap`]
    /// or element in [`AnyValue::Vec`].
    Unsigned16(u16),

    /// `signed 32-bit` value stored for specific key in [`AnyValue::HashMap`]
    /// or element in [`AnyValue::Vec`].
    Signed32(i32),

    /// `unsigned 32-bit` value stored for specific key in [`AnyValue::HashMap`]
    /// or element in [`AnyValue::Vec`].
    Unsigned32(u32),

    /// `signed 64-bit` value stored for specific key in [`AnyValue::HashMap`]
    /// or element in [`AnyValue::Vec`].
    Signed64(i64),

    /// `unsigned 64-bit` value stored for specific key in [`AnyValue::HashMap`]
    /// or element in [`AnyValue::Vec`].
    Unsigned64(u64),

    /// `signed 128-bit` value stored for specific key in [`AnyValue::HashMap`]
    /// or element in [`AnyValue::Vec`].
    Signed128(i128),

    /// `unsigned 128-bit` value stored for specific key in
    /// [`AnyValue::HashMap`] or element in [`AnyValue::Vec`].
    Unsigned128(u128),

    /// `32-bit floating point` value stored for specific key in
    /// [`AnyValue::HashMap`] or element in [`AnyValue::Vec`].
    Float32(f32),

    /// `64-bit floating point` value stored for specific key in
    /// [`AnyValue::HashMap`] or element in [`AnyValue::Vec`].
    Float64(f64),
}
...
impl<T> From<HashMap<String, T>> for AnyValue
where
    T: Into<AnyValue>,
{
        ...
        Self::HashMap(
            value
                ...
                .fold(HashMap::new(), |mut acc, (key, value)| {
                    acc.insert(key, Box::new(value.into()));
                    acc
                }),
        )
    }
...
impl<T> From<Vec<T>> for AnyValue
where
    T: Into<AnyValue>,
{
    fn from(value: Vec<T>) -> Self {
        ...
        Self::Vec(
            value
                .into_iter()
                .fold(Vec::with_capacity(capacity), |mut acc, value| {
                    acc.push(Box::new(value.into()));
                    acc
                }),
        )
    }
